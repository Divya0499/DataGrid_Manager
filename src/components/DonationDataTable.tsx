...
const DonationDataTable: React.FC<DonationDataTableProps> = ({ data, setData, onEdit, onAdd }) => {
  const [searchTerm, setSearchTerm] = useState(""); // State to hold the search term
  const [copiedRow, setCopiedRow] = useState<string | null>(null); // State to hold the ID of the copied row
  const [sortDirection, setSortDirection] = useState<'ascending' | 'descending'>('ascending'); // State to control sort direction
  ...
  const handleSort = useCallback(() => {
    setSortDirection(prevDirection => prevDirection === 'ascending' ? 'descending' : 'ascending'); // Toggle sorting direction
  }, []);
